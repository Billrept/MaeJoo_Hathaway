    ...
    const sortedRows = React.useMemo(() => {
      if (sortConfig.key) {
        const sorted = [...rows].sort((a, b) => {
          ...
          // Handle "change" column separately
          if (sortConfig.key === 'change') {
            // Calculate change for both rows
            ...
            // Compare the calculated change values
            if (aChange < bChange) {
          ...
          // Handle percentage or numeric columns like currentPricing
          if (sortConfig.key === 'currentPricing') {
            aValue = parseFloat(aValue) || 0; // Ensure valid float value
            bValue = parseFloat(bValue) || 0;
          }
    
          // Sort string columns (e.g., ticker)
