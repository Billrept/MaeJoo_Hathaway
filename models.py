...
def arima_prediction(close_prices):
    ...
    model_fit = model.fit()
    predicted_price = float(model_fit.forecast(steps=1)[0])  # Convert to Python float
    return predicted_price


def garch_prediction(close_prices):
    # Convert list to a Pandas Series before calculating returns
    close_prices_series = pd.Series(close_prices)
    returns = close_prices_series.pct_change().dropna() * 100
    
    # Fit the GARCH model on the returns
    garch_model = arch_model(returns, vol='Garch', p=2, q=2)
    garch_fit = garch_model.fit(disp="off")
    
    # Forecast volatility and convert to Python float
